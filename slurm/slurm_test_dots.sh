...
#SBATCH -o slurm.%j.%N.ERM_breeds.mammal-bird-slurm-test-dots.out # STDOUT
#SBATCH -e slurm.%j.%N.ERM_breeds.mammal-bird-slurm-test-dots.err # STDERR
...
srun -N 1 -n 1 --gres=gpu:1 python run_expt.py --shift_type confounder --dataset Breeds --target_name breeds --save_step 10 --save_last --save_best --lr .1 --step_scheduler --scheduler_step 120 --n_epochs 300 --batch_size 128 --weight_decay 1.00E-04 --model resnet18 --train_from_scratch --gamma 0.1 --generalization_adjustment 0 --breeds_dataset_type entity13 --log_dir ./logs/ERM_breeds/mammal-bird-dots/mammal-bird-dots-18 --breeds_color_dots
job_limit 2

srun -N 1 -n 1 --gres=gpu:1 python run_expt.py --shift_type confounder --dataset Breeds --target_name breeds --save_step 10 --save_last --save_best --lr .1 --step_scheduler --scheduler_step 120 --n_epochs 300 --batch_size 128 --weight_decay 1.00E-04 --model resnet50 --train_from_scratch --gamma 0.1 --generalization_adjustment 0 --breeds_dataset_type entity13 --log_dir ./logs/ERM_breeds/mammal-bird-dots/mammal-bird-dots-50 --breeds_color_dots

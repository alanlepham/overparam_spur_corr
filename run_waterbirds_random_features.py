...
def main():
    ...
    # Model
    parser.add_argument(
        "-m", "--model_type", choices=["ridge", "logistic"], required=True
    )
    parser.add_argument("-L", "--Lambda", type=float, default=None)
    # Error
    parser.add_argument(
        "-e", "--error_type", choices=["zero_one", "squared"], required=True
    )
    # Outputs
    ...
    full_data, n_groups = load_waterbirds_data(
        args.features_path, args.metadata_path, frac=args.frac
    )
    erm_error, over_error, under_error = run_random_features_model(
        full_data=full_data,
        n_groups=n_groups,
...
def load_waterbirds_data(features_path, metadata_path, frac=1.0):
    ...
    train_g = (
        2 * metadata[train_mask]["y"].values + metadata[train_mask]["place"].values
    )
    if frac < 1:

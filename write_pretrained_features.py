...
def main():
    ...
    # Settings
    parser.add_argument(
        "-d", "--dataset", choices=dataset_attributes.keys(), required=True
    )
    ...
    parser.add_argument(
        "-m", "--model", choices=model_attributes.keys(), default="resnet18"
    )
    parser.add_argument("--model_path", default=None)
    parser.add_argument("--layers_to_extract", type=int, default=1)
    parser.add_argument(
        "--get_preds_instead_of_features", action="store_true", default=False
    )
    ...
    if not args.get_preds_instead_of_features:
        if args.model == "resnet18":
            model = resnet18(pretrained=True, layers_to_extract=args.layers_to_extract)
        else:
    ...
    if not args.get_preds_instead_of_features:
        ...
        output_path = (
            args.model_path.split(".pth")[0] + "_preds-on_" + args.dataset + ".npy"
        )

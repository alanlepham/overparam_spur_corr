...
def main():
    ...
    # Optimization
    parser.add_argument(
        "--n_epochs", type=int, default=4, help="number of (additional) epochs to run"
    )
    ...
    if resume:
        ...
        if "last_model.pth" in pth_files_full_names:
            ...
            pth_files = sorted(
                [
                    int(f.split("_")[0])
                    for f in pth_files_full_names
                    if f.split("_")[0].isnumeric()
                ]
            )
    ...
    # misc resnet50 pretrain
        ...
        model = InceptionResnetV1(
            pretrained="vggface2", num_classes=n_classes, classify=True
        )

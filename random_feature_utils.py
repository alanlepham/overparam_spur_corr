...
def fit_logistic_regression(X, y, Lambda=None):
    ...
    model = LogisticRegression(
        **penalty_args, fit_intercept=False, solver="lbfgs", max_iter=1e8
    )
    model.fit(X, y)
    return model
...
def compute_error(
    full_data, model, n_groups, error_fn, resample_idx=None, verbose=True
):
    error_log = {}
    ...
    if resample_idx is not None:
        train_x, train_y, train_g = (
            train_x[resample_idx, :],
            train_y[resample_idx],
            train_g[resample_idx],
        )
    group_train_error = []
    for g in range(n_groups):
        g_mask = train_g == g
    ...
    error_log["avg_train_error"] = np.array(group_train_error) @ (
        np.array(group_count) / sum(group_count)
    )
    ...
    error_log["avg_test_error"] = np.array(group_test_error) @ (
        np.array(group_count) / sum(group_count)
    )
...
def run_no_projection_model(
    ...
    over_model = fit_model_fn(
        train_x[resample_idx, :], train_y[resample_idx], **model_kwargs
    )
    over_error = compute_error(
        data, over_model, n_groups, error_fn, resample_idx=resample_idx, verbose=verbose
    )
    # UNDER
    if verbose:
        print("\nUndersampling")
    resample_idx = undersample(train_g, n_groups)
    under_model = fit_model_fn(
        train_x[resample_idx, :], train_y[resample_idx], **model_kwargs
    )
    under_error = compute_error(
        data,
        under_model,
        n_groups,
        error_fn,
...
def run_random_features_model(
    ...
    erm_error = compute_error(
        projected_data, model, n_groups, error_fn, verbose=verbose
    )
    # OVER
    if verbose:
        print("\nOversampling")
    resample_idx = oversample(train_g, n_groups)
    model = fit_model_fn(
        proj_train_x[resample_idx, :], train_y[resample_idx], **model_kwargs
    )
    over_error = compute_error(
        projected_data,
        model,
...
def save_error_logs(outfile, error_log_dict_list, opt_type_list):
    ...
    with open(outfile, "w") as f:
        for opt_type, error in zip(opt_type_list, error_log_dict_list):
...
def plot_double_descent(
    ...
    if figure is not None:
        ...
        fig, ax = plt.subplots(
            2, len(opt_types), figsize=(8, 4), sharex=True, sharey="row"
        )
    for i, opt_type in enumerate(opt_types):
        ax[0, i].semilogx(
            x_axis_values,
            np.mean(train_accs[opt_type], axis=1),
            color=train_color,
            label=train_label,
        )
        ax[1, i].semilogx(
            x_axis_values,
            np.mean(test_accs[opt_type], axis=1),
            color=test_color,
            label=test_label,
        )
        train_label = None
        ...
        for replicate_idx in range(train_accs[opt_type].shape[1]):
            ax[0, i].scatter(
                x_axis_values,
                train_accs[opt_type][:, replicate_idx],
                color=train_color,
                marker=".",
                alpha=0.5,
            )
            ax[1, i].scatter(
                x_axis_values,
                test_accs[opt_type][:, replicate_idx],
                color=test_color,
                marker=".",
                alpha=0.5,
            )
...
def plot_results(
    get_filepath_dict, args_list, N_list, robust, verbose=False, legend_args={}
):
    ...
    for i, args in enumerate(args_list):
        ...
        plot_double_descent(
            ...
            train_label=None,
            test_label=label,
            train_color=cmap((i * 2 + 1) % 20),
            test_color=cmap((i * 2) % 20),
